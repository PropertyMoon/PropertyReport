...
RESEARCH_TASKS = {
    ...
    "property_market": (
        ...
        "(5) If step 4 fails, search the bare address with suburb and postcode plus the word 'sold' — e.g. "
        "\"23 Waterside Drive Springfield Lakes 4300 sold\" — Google often surfaces the price "
        "in a snippet from a news article, PropTrack summary, or real estate aggregator.\n"
        "Only return null for subject_property_last_sale if ALL FIVE queries return zero sold results for this address.\n"
        "(6) Search for 2 recent comparable sales in the same suburb.\n"
        "(7) Search for suburb-level market data (days on market, clearance rate, outlook).\n"
        "Return JSON with: "
        "subject_property_last_sale (object: price (numeric AUD — no $ sign, just the number), "
        "date (string e.g. 'February 2025') — "
        "REQUIRED: only return null if all five search queries above return zero sold results for this address), "
