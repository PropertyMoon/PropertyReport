...
RESEARCH_TASKS = {
    "suburb": (
        "Property: {address}\nState: {state}\n"
        "CRIME DATA: Search '[suburb] [state] crime statistics' and prefer {crime_url} as the primary source. "
        "If {crime_url} has no suburb-level data, use any authoritative source (state police, ABS, suburb profiles). "
        "You MUST return numeric values for the three crime fields — do not return null unless no data exists anywhere.\n"
        ...
        "fall back to any police or ABS source; estimate from context if no exact figure is available), "
        "crime_violent_vs_state_avg_pct (signed integer — percentage delta of suburb's violent crime rate vs state average; positive = worse than average), "
        "crime_property_vs_state_avg_pct (signed integer — same for property crime), "
        "price_history_5yr (list of up to 6 objects: year (int 2021-2026), median_house_price (numeric AUD) — "
        "annual median for the suburb; search realestate.com.au/[suburb] suburb profile for the most recent figures), "
        ...
        "Search '[suburb] [state] median house price 2025 2026 site:realestate.com.au' to get the most current price_growth_5yr and price_history_5yr figures."
    ),
