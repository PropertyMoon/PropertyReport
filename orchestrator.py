...
RESEARCH_TASKS = {
    "suburb": (
        "Property: {address}\nState: {state}\n"
        "STEP 1 — Fetch the realestate.com.au suburb profile page directly. "
        "Construct the URL: https://www.realestate.com.au/neighbourhoods/[suburb-lowercase-hyphenated]-[postcode]-[state-lowercase]/ "
        "(e.g. for Taylors Lakes VIC 3038: https://www.realestate.com.au/neighbourhoods/taylors-lakes-3038-vic/). "
        "From this page extract: median_house_price, median_unit_price, price_growth_5yr, rental_yield, and price_history_5yr. "
        "These values from this page are authoritative — do not override them with search results.\n"
        "STEP 2 — Search '[suburb] [state] crime statistics' and prefer {crime_url} as the primary source. "
        "If {crime_url} has no suburb-level data, use any authoritative source (state police, ABS, suburb profiles). "
        "Return null for crime fields only if no numeric data exists anywhere — do not estimate.\n"
        ...
        "derive from the suburb's crime rate relative to the state average; use {crime_url} first, "
        "fall back to any police or ABS source; null if no numeric data available from any authoritative source), "
        ...
        "price_history_5yr (list of up to 6 objects: year (int 2021-2026), median_house_price (numeric AUD) — from the realestate.com.au suburb profile fetched in Step 1), "
