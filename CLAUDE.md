...
- **Known external-service caveat (verified by direct testing, not assumed from old docs)**: as of this writing, `au-crime-mcp`'s Railway deployment returns 404 on every path (including `/health`) — it appears to be down, not just rate-limited. `au-median-price-mcp` is reachable but its NSW Valuer-General source is currently 403'd (cloud IP blocked) and non-NSW states depend on Domain-via-Scrapfly gated on a postcode + that service's own `SCRAPFLY_API_KEY`. Until these are fixed, the comparator will only show ABS demographics for most suburb pairs — this is expected, not a bug in this repo.
