...
RESEARCH_TASKS_PERPLEXITY = {
    "suburb": (
        ...
        "Prefer official government, council, transport, school, and planning sources first.\n"
        "Use commercial property sites only if official sources are unavailable.\n"
        "Do not assume suburb-level proximity for this specific address.\n"
        "Return only verified facts. If a distance cannot be confirmed from a reliable source, return null.\n\n"
        ...
        "Search '{address} suburb profile median house price' on realestate.com.au or domain.com.au to find year-by-year median house prices. "
        "Also search the suburb name from {address} combined with 'median house price 2021 2022 2023 2024 2025'. "
        "Return up to 6 yearly median house prices for 2021–2026. Only include a year if the figure is explicitly stated in a source.\n\n"
        "STEP 4 — LIFESTYLE AMENITIES\n"
        "Search '{address} nearest supermarket' — return the closest Coles, Woolworths, or Aldi with its name and walking distance.\n"
        "Search 'nearest gym to {address}' — return the closest gym regardless of brand, its distance, and approximate weekly membership cost if publicly listed.\n"
        "Search '{address} nearest park reserve' — return the closest public park or recreation reserve with its name and distance.\n"
        "Search '{address} nearest GP clinic' — return the closest general practice or bulk-billing clinic with its name and distance.\n"
        "Search '{address} nearest hospital' — return only if a hospital is within 10 km.\n"
        "Search 'nearest freeway motorway to {address}' — return the closest major road with its name and driving distance in km.\n"
        "Return only verified results with actual place names and distances confirmed from a source. If a distance cannot be confirmed, return null.\n\n"
        "STEP 5 — CRIME\n"
    ...
    "schools": (
        ...
        "Find the official government primary school catchment for {address}.\n"
        "Use the official state school zone map (e.g. findmyschool.vic.gov.au for VIC, or equivalent) "
        "to confirm whether this address is inside the catchment boundary (in_catchment = true/false).\n"
        "Then return the nearest school name, ICSEA from myschool.edu.au, latest available NAPLAN reading and numeracy, "
        "and walking distance from the property using a street-level source.\n"
    ...
    "transport": (
        "Research the property at {address}, {state}.\n"
        "Use location-aware search and map-based lookups.\n"
        "Prefer official transport agency sources first.\n"
        "Do not infer proximity from the suburb name — confirm actual walking distance to this specific address.\n"
        "Return only verified facts. If a distance or travel time cannot be confirmed, return null.\n\n"
        ...
        "STEP 1 — TRAIN\n"
        "Search 'nearest train station to {address}' to find the station name, line, and CBD travel time.\n"
        "Then search 'walking distance from {address} to [station name]' to confirm the actual street-level "
        "walking distance from this specific address to that station. "
        "Do not assume the suburb station is nearest — many suburbs are 1–4 km from their station. "
        "Return distance_km rounded to 2 decimal places.\n\n"
        "STEP 2 — BUS\n"
        "Search 'bus routes near {address}' using {transport_url} or any official transport source.\n"
        "List only routes that plausibly stop within 1 km of the address.\n\n"
        "STEP 3 — TRAM\n"
        "Confirm tram access only if a tram stop is within 500m of {address}.\n\n"
        "STEP 4 — DRIVE\n"
        "Search 'drive time from {address} to CBD' for peak and off-peak estimates.\n"
        "If either value cannot be verified, return null.\n\n"
        "RETURN JSON WITH THESE KEYS\n"
        "nearest_train (object: name, distance_km, line, cbd_mins), "
