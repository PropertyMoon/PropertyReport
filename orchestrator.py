...
RESEARCH_TASKS = {
    ...
    "property_market": (
        "Property: {address}\n"
        "CRITICAL: You MUST find the last sold price for this specific property. Follow ALL steps.\n\n"
        "STEP 1 — Read the Google search SNIPPET (do this first, it is the most reliable):\n"
        "Search Google for: \"{address} sold\"\n"
        "Google's own search result snippet often shows 'Sold $X,XXX,XXX' or 'Sold on DD Mon YYYY for $X,XXX,XXX' "
        "as plain text — read the price and date DIRECTLY from the snippet text without opening any page. "
        "The snippet typically appears under a realestate.com.au or domain.com.au result and contains "
        "the dollar amount and sold date as readable text.\n\n"
        "STEP 2 — If the snippet in step 1 did not contain the price, search:\n"
        "\"{address} sold realestate.com.au\"\n"
        "Read the price from the Google snippet for this result (look for a dollar amount like '$1,470,000' "
        "near 'Sold' in the snippet text — you do NOT need to open the page).\n\n"
        "STEP 3 — If step 2 snippet has no price, search:\n"
        "\"{address} sold domain.com.au\"\n"
        "Again read from the snippet text.\n\n"
        "STEP 4 — If steps 1-3 all failed, search:\n"
        "\"{address} sold price 2025 OR 2024 OR 2023\"\n\n"
        "STEP 5 — If all above failed, search:\n"
        "\"{address} sold site:onthehouse.com.au OR site:homesales.com.au OR site:pricefinder.com.au\"\n\n"
        "Only return null for subject_property_last_sale if ALL FIVE steps return no dollar amount for this address.\n\n"
        "STEP 6 — Search for 2 recent comparable sales in the same suburb.\n"
        "STEP 7 — Search for suburb-level market data (days on market, clearance rate, outlook).\n\n"
        "Return JSON with: "
