...
| `SENDGRID_API_KEY` + `SENDER_EMAIL` | `SENDER_EMAIL` always required. `email_sender.py` tries providers in order: SendGrid → Brevo → SMTP, falling through on failure or if a key is absent. Production startup requires at least one of `SENDGRID_API_KEY`, `BREVO_API_KEY`, or `SMTP_PASS`. |
| `BREVO_API_KEY` | Brevo (Sendinblue) transactional email HTTP API key — free tier is 300 emails/day forever. Used via a plain HTTPS call (`_send_via_brevo` in `email_sender.py`), so unlike SMTP it works on Railway's Free/Trial/Hobby plans (see `SMTP_*` note below). |
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` | SMTP relay credentials, used only as the last resort when neither `SENDGRID_API_KEY` nor `BREVO_API_KEY` is set (or both fail). Defaults: `SMTP_HOST=smtp.gmail.com`, `SMTP_PORT=587`, `SMTP_USER=SENDER_EMAIL`. **Railway note:** raw SMTP ports (465/587) are blocked outbound on Railway's Free/Trial/Hobby plans — only Pro+ plans (after a redeploy) can use this path there; use `BREVO_API_KEY` instead on lower plans. |
